...
from pybricks.tools import wait, StopWatch  # 待機とタイマーの道具
...
class Robot:
    ...
    async def straight(self, distance, speed=None, acceleration=None, timeout=None):
        ...
        if speed is not None or acceleration is not None:
            self._robot.settings(
                straight_speed=speed if speed is not None else DEFAULT_STRAIGHT_SETTINGS['straight_speed'],
                straight_acceleration=acceleration if acceleration is not None else DEFAULT_STRAIGHT_SETTINGS['straight_acceleration']
            )
    ...
    async def turn(self, angle, rate=None, acceleration=None, timeout=None):
        ...
        if rate is not None or acceleration is not None:
            self._robot.settings(
                ...
                turn_acceleration=acceleration if acceleration is not None else DEFAULT_TURN_SETTINGS['turn_acceleration']
            )
    ...
    async def curve(self, radius, angle, speed=None, acceleration=None, timeout=None):
        ...
        if speed is not None or acceleration is not None:
            self._robot.settings(
                straight_speed=speed if speed is not None else DEFAULT_CURVE_SETTINGS['straight_speed'],
                straight_acceleration=acceleration if acceleration is not None else DEFAULT_CURVE_SETTINGS['straight_acceleration']
            )
